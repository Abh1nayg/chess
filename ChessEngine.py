...
def get_enpassant_square(enpassant_possible):
    """
    Returns the en passant square part of the FEN string.
    """
    if enpassant_possible == ():
        return '-'
    else:
        row, col = enpassant_possible
...
class GameState:
    ...
    def fen(self):
            ...
            for col in range(8):
                ...
                if piece == '--':
                    ...
                    if empty_squares > 0:
                        ...
                        empty_squares = 0
                    fen_string += piece[1].upper() if piece[0] == 'w' else piece[1].lower()
            if empty_squares > 0:
                fen_string += str(empty_squares)
            if row != 7:
                fen_string += '/'
        ...
        # Active color
        fen_string += ' w ' if self.white_to_move else ' b '
        
        # Castling rights
        fen_string += get_castling_rights(self.current_castling_rights)
        
        # En passant square
        fen_string += ' - ' if self.enpassant_possible == () else ' ' + get_enpassant_square(self.enpassant_possible)
        
        # Halfmove clock and fullmove number (not required)
        fen_string += ' 0 1'
    ...
    def checkForPinsAndChecks(self):
        ...
        for j in range(len(directions)):
            ...
            for i in range(1, 8):
                ...
                if 0 <= end_row < self.board.shape[0] and 0 <= end_col < self.board.shape[1]:
    ...
    def getRookMoves(self, row, col, moves):
        ...
        for direction in directions:
            for i in range(1, 8):
                ...
                if 0 <= end_row < self.board.shape[0] and 0 <= end_col < self.board.shape[1]:  # check for possible moves only in boundaries of the board
    ...
    def getBishopMoves(self, row, col, moves):
        ...
        for direction in directions:
            for i in range(1, 8):
                end_row = row + direction[0] * i
                end_col = col + direction[1] * i
                if 0 <= end_row < self.board.shape[0] and 0 <= end_col < self.board.shape[1]:  # check if the move is on board
                    if not piece_pinned or pin_direction == direction or pin_direction == (
                            -direction[0], -direction[1]):
                        end_piece = self.board[end_row][end_col]
                        if end_piece == "--":  # empty space is valid
